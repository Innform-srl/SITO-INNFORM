...
import { GraduationCap, Target, Eye, Shield } from 'lucide-react';
import { useScrollReveal } from '../hooks/useScrollReveal';
import { useRealtimePaths } from '../hooks/useRealtimePaths';
import { useRealtimeCourses } from '../hooks/useRealtimeCourses';

function ProgramCard({ program, index }: { program: typeof basePrograms[0]; index: number }) {
  ...
  return (
    <Link
      ...
      className={`group relative bg-white rounded-3xl shadow-lg hover:shadow-2xl transition-all duration-500 overflow-hidden transform hover:-translate-y-2 flex flex-col reveal-up ${revealed ? 'revealed' : ''}`}
...
const sicurezzaProgram = {
  id: 'sicurezza',
  title: 'Sicurezza sul Lavoro',
  description: 'Formazione obbligatoria D.Lgs. 81/08: Rischio Basso, Medio e Alto, Formazione Generale, Aggiornamenti.',
  color: 'from-red-600 to-orange-500',
  icon: Shield,
  features: ['Attestati Validi', 'D.Lgs. 81/08', 'Docenti Certificati'],
};

export function Programs() {
  const { ref: headerRef, revealed: headerRevealed } = useScrollReveal();
  const { paths } = useRealtimePaths();
  const { courses } = useRealtimeCourses();

  // Mostra la card Sicurezza solo se c'è almeno un corso sicurezza pubblicato
  const hasSicurezzaPublished = useMemo(() => {
    // Controlla nel learning path SIC se ha corsi pubblicati
    const sicPath = paths.find(p => p.code === 'SIC' || p.code === 'SICUREZZA' || p.code.startsWith('SIC'));
    if (sicPath && sicPath.courses.some(c => c.status === 'published')) {
      return true;
    }
    // Controlla anche nei corsi standalone con codice SICUREZZA
    return courses.some(c => c.code.startsWith('SICUREZZA'));
  }, [paths, courses]);

  const programs = useMemo(() => {
    if (hasSicurezzaPublished) {
      return [...basePrograms, sicurezzaProgram];
    }
    return basePrograms;
  }, [hasSicurezzaPublished]);
  ...
  return (
    <section id="programmi" className="py-24 bg-gradient-to-b from-gray-50 to-white relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className={`grid md:grid-cols-2 ${programs.length === 4 ? 'lg:grid-cols-4' : 'lg:grid-cols-3'} gap-6 max-w-7xl mx-auto`}>
